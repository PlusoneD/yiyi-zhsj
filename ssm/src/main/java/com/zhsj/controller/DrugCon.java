...
public class DrugCon {
    ...
    @RequestMapping("/Manager_Drug")
    public String index() {
        return "manager/Manager_Drug";
    }
    @RequestMapping("/Manager_Drug_UpdateForm")
    public String index2() {
        return "manager/Manager_Drug_UpdateForm";
    }
    @RequestMapping(value = "/getDrugById", method = RequestMethod.POST)
    @ResponseBody
    public Drug getById(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("UTF-8");
        String id = request.getParameter("id");
        Drug d = drug.queryById(id);
